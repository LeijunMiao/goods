...
namespace goods
{
    public partial class InOrderView : Form
    {
        listCtrl ctrl = new listCtrl();
        listParmas lp = new listParmas();
        ...
        private void initData(string num)
        {
            ...
            lp.warehouse = Convert.ToInt32(dt.Rows[0]["warehouse"]);
            if (dt.Rows[0]["position"] == DBNull.Value) lp.position = null;
            else lp.position = Convert.ToInt32(dt.Rows[0]["position"]);

            lp.id = Convert.ToInt32(dt.Rows[0]["gid"]);
            ...
            {
                int can = ctrl.inOrderStockChanged(lp.id);
                if (can != 0)
                {
                    toolStripButton2.Enabled = false;
                }
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确定删除?", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                var msg = ctrl.delInOrder(lp, listM);
                if (msg.Code == 0)
                {
